...
- **The Sat 11/07 near-miss in `demo/documents/05-care-log.md` contradicts its own text.** The demo notes call it "stability language with no intervention anywhere nearby", but the entry reads `Assisted wash/dress. Prompted meds. Good day, no concerns…` — an intervention 12 characters from the stability phrase, in the same visit. Any detector honouring the brief's same-source co-occurrence rule fires there, and correctly so. Before scoring this section, amend the Saturday entry to be genuinely intervention-free (e.g. drop `Assisted wash/dress. Prompted meds.` from that line) or place the near-miss on a line with no intervention. Lane C covers the intended near-miss by testing the Saturday stability sentence in isolation, and its well-managed tests derive their transcript from this document at test time — so re-run `pnpm test` after amending it.
