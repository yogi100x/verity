/**
 * Audio classification for the voice upload seam
 * (`app/api/voice/upload/route.ts`), mirroring `lib/ai/documents.ts` in
 * style: bytes are sniffed, never trusted from a filename or a client
 * `Content-Type` header. This module deliberately does not import from
 * `documents.ts` — audio and document sniffing are separate concerns with
 * separate magic-byte tables, and keeping them apart means neither can grow
 * an accidental dependency on the other's territory.
 *
 * `RIFF` is the same container prefix WAV and WebP both use, which is why
 * `detectAudioMediaType` (audio) and `detectImageMediaType` (documents.ts,
 * for images) each check a *second* magic sequence at offset 8 before
 * committing to a media type — offset 0 alone is ambiguous between the two.
 *
 * Two container families share a prefix with something that is NOT audio, and
 * both discriminators here are load-bearing rather than decorative:
 *   - `RIFF` at offset 0 is WAV *or* WebP (an image). Offset 8 decides.
 *   - `ftyp` at offset 4 is the ISO base media file format, shared by MP4/M4A
 *     *and* by HEIC/HEIF/AVIF **images** and other non-audio families. The
 *     brand at offset 8 decides, against an allowlist.
...
/**
 * `MediaRecorder` clips from a browser mic capture are a few minutes at most
 * (the demo's spoken prompt is short by design — see
 * `docs/lanes/lane-e-voice.md` rung 2's fixed `<Record maxLength="60">`).
 * WebM/Opus at a typical voice bitrate (~24–32 kbps mono) runs about 200KB a
 * minute, so 4MB is roughly twenty minutes of speech — generous headroom for a
 * clip the demo caps at 60 seconds.
 *
 * The ceiling is 4MB and not larger for a deployment reason, not a taste one:
 * a Vercel serverless function rejects a request body over 4.5MB at the edge,
 * before this route runs, and the client would see an opaque platform error
 * instead of this endpoint's 413. A limit we can actually enforce and name in
 * our own error message is worth more than a bigger number we cannot.
 */
 ...
 * (`avif`, `avis`) images, JPEG 2000 (`jp2 `) and QuickTime (`qt  `) all carry
 * the identical box at the identical offset. An allowlist rather than a
 * denylist on purpose — a brand nobody has enumerated must fail closed, and a
 * brand that genuinely needs admitting is a one-line change here plus a test,
 * not a silent hole. Pure-video brands (`avc1`, `mp4v`) are deliberately
 * absent: `MediaRecorder` does not emit them for an audio-only capture.
 *
...
/** Largest plausible `ftyp` box: 8 bytes of header plus a long compatible-brand
 *  list. Real ones run 16–32 bytes; 1024 is generous headroom while still
 *  rejecting an arbitrary file whose bytes 4–7 happen to spell "ftyp" and whose
 *  leading four bytes are not a credible box length. */
...
const WEBP_MAGIC = [0x57, 0x45, 0x42, 0x50]; // "WEBP", at offset 8 — NOT audio

/**
 * True for a *valid* MPEG audio frame header, not merely a sync word. A raw MP3
 * stream with no ID3 tag starts directly with a frame header rather than "ID3",
 * and the sync word alone (`FF` plus three set bits) is only 11 bits — far too
 * weak a signal to store an arbitrary file on. So the reserved encodings in the
 * same three bytes are rejected too:
 ...
 * `version == 01` is reserved, `layer == 00` is reserved, bitrate index `1111`
 * is "bad" and sample-rate index `11` is reserved — all four are impossible in
 * a real stream, so bytes carrying them are not an MP3. This is what keeps
 * `FF E0 00 …` style near-misses out of storage.
 */
...
export function detectAudioMediaType(bytes: Uint8Array): AudioMediaType | null {
  ...
  // `ftyp` at offset 4 is necessary but nowhere near sufficient — the brand at
  // offset 8 is what separates an M4A recording from a HEIC photo.
  ...
  // WAV must be checked before/independent of any generic RIFF handling:
  // RIFF+WAVE is audio, RIFF+WEBP (checked, rejected) is not.
  if (startsWith(bytes, RIFF_MAGIC) && startsWith(bytes, WAVE_MAGIC, 8)) return 'audio/wav';
  return null;
}

/** Throw AudioTooLargeError if the byte length exceeds MAX_AUDIO_BYTES. */
export function assertWithinAudioLimit(byteLength: number, label: string): void {
  if (byteLength > MAX_AUDIO_BYTES) {
    throw new AudioTooLargeError(
      `${label} is ${byteLength} bytes, over the ${MAX_AUDIO_BYTES}-byte limit ` +
        `(${MAX_AUDIO_BYTES / (1024 * 1024)}MB).`,
    );
  }
}

/** Extension for a media type, for the storage object name. */
export function extensionFor(mediaType: AudioMediaType): string {
  switch (mediaType) {
    case 'audio/webm':
      return 'webm';
    case 'audio/ogg':
...
/** Classify raw uploaded bytes, or throw UnsupportedAudioError. */
export function classifyAudioUpload(bytes: Uint8Array): AudioMediaType {
  const mediaType = detectAudioMediaType(bytes);
  if (mediaType !== null) return mediaType;
  throw new UnsupportedAudioError(
    `Could not classify upload (${bytes.byteLength} bytes): not a recognised ` +
      'audio container (WebM, Ogg, MP4/M4A, MP3, WAV), or shorter than the ' +
      `${MIN_SNIFF_BYTES} bytes classification needs.`,
  );
}
